...
dev-spectrogram:
	...
	uv run python -m src.scripts.devel --config configs/baseline.yaml --test spectrogram --n-samples 1

dev-envelope:
	# Test signal processing (envelope patterns and filter banks)
	uv run python -m src.scripts.devel --config configs/baseline.yaml --test signal
...
explore-compute:
	...
	uv run python -m src.scripts.explore --config configs/baseline.yaml --compute-stats

explore-spectrograms:
	# Generate spectrograms (skipping problematic files by default)
	uv run python -m src.scripts.explore --config configs/baseline.yaml --compute-spectrograms --max-per-label 5
...
train:
	uv run python -m src.scripts.train --config configs/baseline.yaml

evaluate:
	uv run python -m src.scripts.evaluate --config configs/baseline.yaml
